...
#### KEGG Mapper
```{r}
WCFS1_final_names <- c(rownames(head(WCFS1_sorted, 5)), rownames(tail(WCFS1_sorted, 5)))
WCFS1<-data.frame(WCFS1_final_names)
WCFS1$logFC<-c((head(WCFS1_sorted, 5)$logFC), (tail(WCFS1_sorted, 5)$logFC))
NC8_final_names <- c(rownames(head(NC8_sorted, 5)), rownames(tail(NC8_sorted, 5)))
NC8<-data.frame(NC8_final_names)
NC8$logFC<-c((head(NC8_sorted, 5)$logFC), (tail(NC8_sorted, 5)$logFC))
```
...
#### K-means
```{r}
km <- kmeans((NC8final %>% select(-(logCPM:FDR))), centers = 4)
fviz_cluster(km, data = (NC8final %>% select(-(LR:FDR))))
```
## Discussie
K-means cluster is niet consistent maakt elke keer andere clusters.
